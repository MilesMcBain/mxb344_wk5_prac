...
ggplot(data=exploratory_set) + 
  geom_bar(aes(x=Pclass, fill=factor(Survived)), position="dodge")
...
train <-
  train %>%
  separate(col = Name, into = c("LastName","Title","FirstName"), sep="[\\.,]") %>%
  mutate(Title = trimws(Title),
         Title = ifelse(Title %in% c("Capt","Col","Don","Dr","Jonkheer","Lady","Major","Mme","Mlle","Rev","Sir","the Countess"),
                  yes = "Unknown", 
                  no = Title)) %>%
...
train_split <-
  caret::createDataPartition(y = train$Survived, p = 0.6, list = FALSE) 

train_set <- train[train_split,]
val_set <- train[-train_split,]

levels(train_set$Title)
levels(val_set$Title)
```


### Model Fitting
```{r}
binom_fit1 <- glm(data=train_set,
...
```{r}
train_preds <- predict(binom_fit1,type = "response")
...
```{r}
#fit and compare
binom_fit_title <- glm(data=train_set,
                       family=binomial(link="logit"),
                       formula = Survived ~ Sex + Pclass + Title)

anova(binom_fit1, binom_fit_title, test="Chisq")
```
...
```{r}
val_preds_title <- predict(binom_fit_title,type = "response", newdata = val_set)
val_preds_title[val_preds_title > 0.5] <- 1
val_preds_title[val_preds_title <= 0.5] <- 0

caret::confusionMatrix(data = val_preds_title, reference = val_set$Survived)
